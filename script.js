...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const navbar = document.querySelector('.navbar'); // Get the main navbar to add/remove classes

    hamburgerMenu.addEventListener('click', function() {
        navbarMenu.classList.toggle('active'); // Toggles the visibility of the menu
        navbar.classList.toggle('menu-open'); // Toggles a class on the navbar for overall layout adjustments
    ...
    // Optional: Close menu when a menu item is clicked (useful for single-page apps)
    navbarMenu.querySelectorAll('a').forEach(item => {
        item.addEventListener('click', function() {
            if (window.innerWidth <= 768) { // Only close on mobile/tablet
    ...
    // Corrected Scroll Button Logic - NOW INSIDE THE MAIN DOMContentLoaded
    // This part will now execute correctly.
    const scrollButtons = document.querySelectorAll('.scroll-button');
    scrollButtons.forEach(button => {
        button.addEventListener('click', function(event) {
            event.preventDefault(); // Prevent default button behavior
            const targetId = this.dataset.target; // Get the ID from data-target (e.g., "inquiryForm")
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentSlide = 0;
    let slideInterval;
    const typingDuration = 3000; // Duration of the typing animation in ms

    function showSlide(index) {
        slides.forEach((slide, i) => {
            slide.classList.remove('active');
            // Remove typing effect classes from all slogans
            const typingTextSpan = slide.querySelector('.typing-text');
            if (typingTextSpan) {
                typingTextSpan.classList.remove('typing-text-active');
            }
        });
        ...
        // Apply typing effect to the active slogan
        const activeTypingTextSpan = slides[index].querySelector('.typing-text');
        if (activeTypingTextSpan) {
            // Force reflow to restart animation
            void activeTypingTextSpan.offsetWidth;
            activeTypingTextSpan.classList.add('typing-text-active');
        }
    }
    ...
    function startAutoSlide() {
        ...
        // Set interval for slide transition + typing duration
        slideInterval = setInterval(() => {
            nextSlide();
        }, 5000 + typingDuration); // 5 seconds for slide + typing duration
    }
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            currentSlide = slideIndex; // Update currentSlide to reflect manual selection
